...
def tenant_list(request, paginate=False, marker=None, domain=None, user=None,admin=True):
    manager = VERSIONS.get_project_manager(request, admin=admin)
...
def user_list(request, project=None, domain=None, group=None,admin=True):
    if VERSIONS.active < 3:
    ...
    users = keystoneclient(request, admin=admin).users.list(**kwargs)
...
def user_update_own_password(request, origpassword, password,email=None):
    client = keystoneclient(request, admin=False)
    if VERSIONS.active < 3:
        client.user_id = request.user.id
        return client.users.update_own_password(origpassword, password)
    else:
        return client.users.update(request.user.id, password=password,email=email)
